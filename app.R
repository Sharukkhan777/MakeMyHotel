...
my_model <<- readRDS("classi_model.rds")
#========================================
#========================================
#------------HELPERS FUNCTION------------
#========================================
...
server <- function(input, output, session) {
  ...
  observeEvent(input$gotopage7,{
    ...
    my_model <<- readRDS("classi_model.rds")
    print("model read")
    data_test <- read.csv("others/data_input.csv")
    print("data read")
    prediction <- predict(my_model, newdata = as.data.frame(ml_data))
    print(prediction)
    output$Rating_out <- renderText({as.character(prediction)})
    output$rating <- renderDataTable({ml_data[1:20]},
